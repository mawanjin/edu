...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/edu/indeximg/">首页顶部图片列表</a></li>
		<li class="active"><a href="${ctx}/edu/indeximg/form?id=${indeximg.id}">首页顶部图片<shiro:hasPermission name="edu:indeximg:edit">${not empty indeximg.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="edu:indeximg:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	...
	<form:form id="inputForm" modelAttribute="indeximg" action="${ctx}/edu/indeximg/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span id="uploadBtn" class="btn btn-success"  <c:if test="${not empty indeximg.img}">style="display: none"</c:if>>
                            <i class="icon-plus icon-white"></i>
                            <span>上传图片</span>
				</span>
				<img id="portrait" <c:if test="${not empty indeximg.img}">src="${ctxImg}/${indeximg.img}"</c:if> />

				<c:if test="${not empty indeximg.img}">
		...
		<div class="control-group">
			<label class="control-label" for="name">顺序:</label>
			<div class="controls">
				<form:input path="porder" htmlEscape="false" maxlength="200" class="required digits"/>
			</div>
		</div>
		...
		<div class="form-actions">
			<shiro:hasPermission name="edu:indeximg:edit">
