...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/edu/teacher/">名师列表</a></li>
		<li class="active"><a href="${ctx}/edu/teacher/form?id=${teacher.id}">名师<shiro:hasPermission name="edu:teacher:edit">${not empty teacher.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="edu:teacher:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	...
	<form:form id="inputForm" modelAttribute="teacher" action="${ctx}/edu/teacher/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span id="uploadBtn" class="btn btn-success"  <c:if test="${not empty teacher.img}">style="display: none"</c:if>>
                            <i class="icon-plus icon-white"></i>
                            <span>上传图片</span>
				</span>
				<img id="portrait" <c:if test="${not empty teacher.img}">src="${ctxImg}/${teacher.img}"</c:if> />
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<tags:ckeditor replace="content" uploadPath="/edu/teacher" />
			</div>
		</div>
