...
<ul class="nav nav-tabs">
	<li><a href="${ctx}/edu/personalQuestion/">个人问答列表</a></li>
	<li class="active"><a href="${ctx}/edu/personalQuestion/form?id=${personalQuestion.id}">个人问答<shiro:hasPermission name="edu:personalQuestion:edit">${not empty personalQuestion.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="edu:personalQuestion:edit">查看</shiro:lacksPermission></a></li>
</ul>
...
<form:form id="inputForm" modelAttribute="personalQuestion" action="${ctx}/edu/personalQuestion/save" method="post" class="form-horizontal">
	<form:hidden path="id"/>
	...
	<div class="control-group">
		...
		<div class="controls">
				...
				<span id="uploadBtn" class="btn btn-success"  <c:if test="${not empty personalQuestion.img}">style="display: none"</c:if>>
                            <i class="icon-plus icon-white"></i>
                            <span>上传图片</span>
				</span>
			<img id="portrait" <c:if test="${not empty personalQuestion.img}">src="${ctxImg}/${personalQuestion.img}"</c:if> />

			<c:if test="${not empty personalQuestion.img}">
	...
	<div class="control-group">
		<label class="control-label" for="summary">类别:</label>
		<div class="controls">
			<select name="questionCategory.id">
			<c:forEach items="${categorys}" var="category">
				<option value="${category.id}" <c:if test="${category.id eq personalQuestion.questionCategory.id}">selected</c:if> >${category.name}</option>
			</c:forEach>
			</select>
		</div>
	</div>
	...
	<div class="control-group">
		...
		<div class="controls">
			...
			<tags:ckeditor replace="content" uploadPath="/edu/personalQuestion" />
		</div>
	</div>

	<div class="form-actions">
		<shiro:hasPermission name="edu:personalQuestion:edit">
