...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#uploadBtn1").on("click",function(event){
				isBadge = true;
				$("#oUploadBtn").trigger("click");
				$("#uploadTable").show();
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/edu/school/">学校列表</a></li>
		<li class="active"><a href="${ctx}/edu/school/form?id=${school.id}">学校<shiro:hasPermission name="edu:school:edit">${not empty school.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="edu:school:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	...
	<form:form id="inputForm" modelAttribute="school" action="${ctx}/edu/school/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span id="uploadBtn" class="btn btn-success"  <c:if test="${not empty school.img}">style="display: none"</c:if>>
                            <i class="icon-plus icon-white"></i>
                            <span>上传图片</span>
				</span>
				<img id="portrait" <c:if test="${not empty school.img}">src="${ctxImg}/${school.img}"</c:if> />
		...
		<!--校徽-->
		<div class="control-group">
			<label class="control-label" for="badge">校徽:</label>
			<div class="controls">

				<span id="uploadBtn1" class="btn btn-success"  <c:if test="${not empty school.badge}">style="display: none"</c:if>>
                            <i class="icon-plus icon-white"></i>
                            <span>上传图片</span>
				</span>
				<img id="badgeImg" <c:if test="${not empty school.badge}">src="${ctxImg}/${school.badge}"</c:if> />

				<c:if test="${not empty school.badge}">
					<button id="delbadgeBtn1"  class="btn btn-danger"  >
						<i class="icon-trash icon-white"></i>
						<span>删除</span>
					</button>
					<script>

						$("#delbadgeBtn1").on("click",function(event){
							isBadge = true;
							$("#uploadBtn1").show();
							$("#delbadgeBtn").hide();
							$("#badge").attr("src","");
				...
				<button id="delBadgeBtn"  class="btn btn-danger" style="display: none;">
					<i  class="icon-trash icon-white"></i>
					<span>删除</span>
				</button>
			</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<tags:ckeditor replace="content" uploadPath="/edu/school" />
			</div>
		</div>
