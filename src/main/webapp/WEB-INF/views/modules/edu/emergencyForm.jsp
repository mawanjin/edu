	...
	<form:form id="inputForm" modelAttribute="emergency" action="${ctx}/edu/emergency/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span id="uploadBtn" class="btn btn-success"  <c:if test="${not empty emergency.img}">style="display: none"</c:if>>
                            <i class="icon-plus icon-white"></i>
                            <span>上传图片</span>
				</span>
				<img id="portrait" <c:if test="${not empty emergency.img}">src="${ctxImg}/${emergency.img}"</c:if> />

				<c:if test="${not empty emergency.img}">
		...
		<div class="control-group">
            ...
            <div class="controls">
                <form:radiobutton path="top" value="0" checked="checked"   />普通
                <form:radiobutton path="top" value="1" />置顶
            </div>
        </div>
